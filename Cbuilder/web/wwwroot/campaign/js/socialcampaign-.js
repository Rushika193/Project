        ...
        var SocialCampaign = {
            ...
            BindGridEvents: function () {
                $('.camp-edit').off('click').on('click', function () {
                    ...
                    location.href = `/dashboard/campaign/create/${SageFrameCurrentCulture}/${campaignID}`;
                });
            ...
            BindCampaignDetailResponse: function (data) {
                ...
                if (statusCode !== 1) {
                    ...
                    if (campaignObj != null) {
                        ...
                        html += `<div class="card-body"><div class='sfFormwrapper'><div class="sfFieldset Mb-10"> <span class="campaign-status ${campaignObj.status}">${campaignObj.status}</span></div><h3 class='campaign-title gray-900 Mb-15 Mt-0'>${campaignObj.campaignTitle}</h3>
                                <div class="sfFieldset">
                                    <span class="key">Published Date: </span>
                                    <span class="value"> ${campaignObj.scheduledOn}</span>
                                    </div>
                                <div class='sfFieldset '>
                                    <span class='key'>Platform: </span>
                                    <span class='value color-primayr fw-600'>${apps}</span>
                                </div>`;
                        ...
                        $.each(campaignObj.apps, function (index, post) {
                            let postHtml = `<div class='campaign-post'>
                                              <div class="sfFieldset  ${post.appName}"><span class="platform-icon"><i class='fa fa-facebook'></i> </span>
                                                <span>${post.pageName}</span></div>
                                               `;
                            if (post.appName === 'twitter') {
                                postHtml += `
                                                <div class='campaign-info sfFieldset'>
                                                    <span class='info-title'>Twitter activity is not available</span>
                                                    <span class='info-content'>To check the activity click <a href='https://twitter.com/i/web/status/${post.postID}' target='_blank'>here</a></span>
                                                </div>
                                            `;
                            }
                                ...
                                postHtml += `
                                                <div class='campaign-info sfFieldset'>
                                                    <span class='info-content'>  <a href='https://www.facebook.com/${targetID}/posts/${postID}' class="btn round primary-outline" target='_blank'>View Post</a></span>
                                                </div>
                                            `;

                                let analytics = post.analytics;

                                postHtml += `<div class='campaign-analytics'> <div class='section flex Mb-25'>`;
                                ...
                                postHtml += ` <div class='impressions view-table Pt-15 Pb-25 sfCol fw-500'>
                                                ...
                                                <span class='key  d-block t-center'>Total Impression </span></div></div></div>
                                            `;
                                postHtml += ` <div class="sfRow counters"><div class="impression-types sfCol-6"><div class='analytics-sub  sfFieldset'>
