...
window.fbControls.push(function (controlClass) {
     ...
     * Sage Icon class
     */
    class controlSageIcon extends controlClass {
         ...
         */
        onRender() {
            let $iconList = $('#iconlist-' + this.config.name);
            let $val = $('#' + this.config.name);
            let $iconHolder = $('#sageicon-' + this.config.name);
            let $sageIconSearch = $('#sageiconsearch-' + this.config.name);
            let $iconDel = $('#sageicondel-' + this.config.name);
            $sageIconSearch.off().on('keyup', function () {
                let v = $(this).val();
                $iconList.find('li').each(function () {
                    let $this = $(this);
                    let dataClass = $this.find('i').attr('data-class');
                    let pos = dataClass.indexOf(v);
                    if (pos < 0) {
                        $this.hide();
                    } else {
                        $this.show();
            ...
            $iconList.off('click').on('click', 'li', function () {
                let iconCls = $(this).find('i').attr('data-class');
                $val.val(iconCls);
