...
window.fbControls.push(function (controlClass) {
     ...
     * Sage Icon class
     */
    var controlSageIcon = function (_controlClass) {
        _inherits(controlSageIcon, _controlClass);

        function controlSageIcon() {
            _classCallCheck(this, controlSageIcon);

            return _possibleConstructorReturn(this, (controlSageIcon.__proto__ || Object.getPrototypeOf(controlSageIcon)).apply(this, arguments));
        }

        _createClass(controlSageIcon, [{
            key: 'configure',
            ...
            value: function onRender() {
                var $iconList = $('#iconlist-' + this.config.name);
                var $val = $('#' + this.config.name);
                var $iconHolder = $('#sageicon-' + this.config.name);
                var $sageIconSearch = $('#sageiconsearch-' + this.config.name);
                var $iconDel = $('#sageicondel-' + this.config.name);
                $sageIconSearch.off().on('keyup', function () {
                    var v = $(this).val();
                    $iconList.find('li').each(function () {
                        var $this = $(this);
                        var dataClass = $this.find('i').attr('data-class');
                        var pos = dataClass.indexOf(v);
                        if (pos < 0) {
                            $this.hide();
                        } else {
                            $this.show();
                ...
                if (value) {
                    $val.val(value);
                    $sageIconSearch.val(value);
                    $iconHolder.html('<i class="fa ' + value + '"></i>');
                    $iconDel.show();
                }
                $iconList.off('click').on('click', 'li', function () {
                    var iconCls = $(this).find('i').attr('data-class');
                    $val.val(iconCls);
                    $iconHolder.html('<i class="fa ' + iconCls + '"></i>');
                    $sageIconSearch.val(iconCls);
                    $val.trigger('change');
                    $iconDel.show();
                });
                $iconDel.off('click').on('click', function (e) {
                    e.preventDefault();
        ...
        return controlSageIcon;
    }(controlClass);

    // register this control for the following types & text subtypes


    controlClass.register('sageIcon', controlSageIcon);
    return controlSageIcon;
});