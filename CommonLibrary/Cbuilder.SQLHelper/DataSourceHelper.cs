...
namespace SQLHelper
{
    ...
    public class DataSourceHelper
    {
        ...
        public static ArrayList GetPropertyInfo(Type objType, DataRow dataRow)
        {

            // Use the cache because the reflection used later is expensive
            ArrayList objProperties = null;
            DataColumnCollection dc = dataRow.Table.Columns;
            ...
            if (objProperties == null)
            {
                ...
                foreach (PropertyInfo objProperty in objType.GetProperties())
                {
                    if (dc.Contains(objProperty.Name))
                        objProperties.Add(objProperty);
                }
            }
            return objProperties;
        ...
        private static T CreateObject<T>(DataRow dr)
        {
            ...
            int intProperty;
            T objObject = Activator.CreateInstance<T>();
            ArrayList objProperties = GetPropertyInfo(objObject.GetType(), dr);
            int counter = objProperties.Count;
